...
description: "Cat-first, dog-friendly social commerce marketplace and community design system for Google Stitch / DESIGN.md."
...
4. **Neutral Theme** for Creator Dashboard and Admin.

Critical rule: one screen uses exactly one theme. Do not mix theme colors.
...
Use for Creator Dashboard, Admin, moderation, user management, report handling, and audit logs.
...
- Admin screens use Neutral tokens only.
...
Typography should feel modern and friendly, but not childish. Consumer UI can be softer. Market, Orders, Dashboard, and Admin must prioritize trust, readability, and scanability.
...
- Do not make Admin typography playful.
...
- Admin uses left sidebar and top header.
- Sidebar width: 240px.
- Header height: 64px.
- Main content padding: 24px.
- Table row height: 48px.
- Admin layouts favor data density, filters, tables, and audit trails.
...
- Avoid heavy shadows.
- Keep admin UI restrained and professional.
...
- Admin tables and controls: 4px radius.
- Admin should not look cute or toy-like.
...
Admin rules:
- Admin must not look cute.
- Admin must not borrow Honey or Mint colors.
- Admin must use restrained neutral UI.
- High-risk actions require confirmation.
- Every moderation action must create an Audit Log item.
...
- Use Neutral only for Dashboard and Admin.
...
- Keep admin pages professional and restrained.
...
- Do not make Admin cute or playful.
